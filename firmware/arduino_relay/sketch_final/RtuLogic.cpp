...
void RtuLogic_init()
{
  for(int pin = 0; pin < MS_OUT_NUM_CHANNELS; pin++){
    ...
    digitalWrite(ms_output_array[pin], LOW);        // sets the digital pins off (green led on)
  }
  ...
  relay[0][TYPE_RELAY_INCOMING_INDEX].deviceType = TYPE_RELAY_INCOMING;  // incoming Relay
  relay[0][TYPE_MEASURE_INDEX].deviceType = TYPE_MEASURE;         // Mesurement device (voltage)
  relay[0][TYPE_RELAY_OUTGOING_INDEX].deviceType = TYPE_RELAY_OUTGOING;  // outgoing Relay
  relay[0][TYPE_RELAY_OUTGOING_INDEX+1].deviceType = TYPE_RELAY_OUTGOING;  // outgoing Relay
  relay[0][TYPE_RELAY_OUTGOING_INDEX+2].deviceType = TYPE_RELAY_OUTGOING;  // outgoing Relay

  relay[1][TYPE_RELAY_INCOMING_INDEX].deviceType = TYPE_RELAY_INCOMING;  // incoming Relay
  relay[1][TYPE_MEASURE_INDEX].deviceType = TYPE_MEASURE;         // Mesurement device (voltage)
  relay[1][TYPE_RELAY_OUTGOING_INDEX].deviceType = TYPE_RELAY_OUTGOING;  // outgoing Relay
  relay[1][TYPE_RELAY_OUTGOING_INDEX+1].deviceType = TYPE_RELAY_OUTGOING;  // outgoing Relay
  relay[1][TYPE_RELAY_OUTGOING_INDEX+2].deviceType = TYPE_RELAY_OUTGOING;  // outgoing Relay
  ...
  for(int i = 0; i < RELAY_ROWS; i++){
    for (int j = 0; j < RELAY_AMOUNT; j++) {
      ...
      relay[i][j].breakerEvent = BREAKER_OPEN;       // 
